...
/*This function accepts the element and the checks if the user has entered the prn as digits, if not then asks the user to input again the correct value
after entering correct value the function finds the prn and then frees it.*/
void del_by_ele(struct student **head)
{
    int ele, flag = 0;
    ...
    do{
        ...
        if(isdigit(ele)){
            struct student *trav = head;//Local pointer var.
            ...
            //after traverse if the traverse pointer is on the last node then call the delete at end function.
            if(trav->next == NULL){
                del_at_end(head);
            }// if ends
            else{
            ...
            flag = 1;
            printf("\n\nYou entered wrong input enter a valid input\n");
        }// else ends


    }while(flag == 1);//Do while loop ends

}//delete at element ends